...
class PropiedadController extends Controller
{
    ...
    public function edit($id){
        ...
        return view('admin.propiedades.edit')->with(compact('propiedad','pisos','incluye','adicional','videos_alrededor','fotos_alrededor','videos_areas','fotos_areas','videos','fotos'));
    }
    ...
    public function update(Request $request, $id){
        try{
            ...
            $pro_incluye = Pro_Incluye::where('id_propiedad',$propiedad->id)->first();
            $pro_incluye->id_propiedad=$propiedad->id;
            $pro_incluye->comedor=$request->comedor;
            $pro_incluye->salon_juegos=$request->salon_juegos;
            $pro_incluye->area_parrillas=$request->area_parrillas;
            ...
            $pro_adicional = Pro_Adicional::where('id_propiedad',$propiedad->id)->first();
            $pro_adicional->id_propiedad=$propiedad->id;
            $pro_adicional->agua=$request->agua;
            $pro_adicional->fibra=$request->fibra;
            $pro_adicional->desague=$request->desague;
            ...
            DB::commit();
            return redirect()->route('activos.index')->with('mensaje','Propiedad editada correctamente!');
        }
            catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'NO SE EDITÓ LA PROPIEDAD');
        }
    }
    ...
    public function storeDistribucion(Request $request){
        $pro_distribucion = new Pro_Distribucion();
        $pro_distribucion->id_propiedad=$request->id_propiedad;
        $pro_distribucion->pisos=$request->piso;
        if ($request->file('imagen_piso')) {
            $file = $request->file('imagen_piso');
            $name = 'propiedad_pisos'.'_' . time() . '.' . $file->getClientOriginalExtension();
            $path = public_path() . '/propiedad/pisos/';
            $file->move($path, $name);
            $pro_distribucion->imagen = $name;
           
        }
        $pro_distribucion->save();
        $pisosData = json_decode($request->pisosData);
        if($pisosData!="[]"){
            for ($i = 0; $i < count($pisosData); $i++) {
                $detalle_piso = new Piso();
                $detalle_piso->id_pro_distribucion = $pro_distribucion->id;
                $detalle_piso->tipo = $pisosData[$i]->tipoPiso;
                $detalle_piso->area = $pisosData[$i]->areaPiso;
                $detalle_piso->dormitorios = $pisosData[$i]->dormitoriosPiso;
                $detalle_piso->banios = $pisosData[$i]->baniosPiso;
                $detalle_piso->n_cuarto = $pisosData[$i]->cuartosPiso;
                $detalle_piso->save();
            }
        }
        return response()->json(['message' => 'Piso subido exitosamente']);
    }

    public function deleteDistribucion(Request $request){
        $id= $request->id;
        $pro_distribucion = Pro_Distribucion::findOrFail($id);
        $detalle_distribucion=Piso::where('id_pro_distribucion',$pro_distribucion->id)->get();
        if(isset($detalle_distribucion)){
            for ($i = 0; $i < count($detalle_distribucion); $i++) {
                $detalle_distribucion[$i]->delete();
            } 
        }
        $pro_distribucion->delete();  
